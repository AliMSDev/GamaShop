...
// this is products component of home page
const Products = (props) => {
    ...
    const addtoBasket = (id) => {
        // give home data to newdata variable
        const newdata = [
            ...props.data
        ]
    ...
    // conditional rendering if home data is not undefined then return this component
    if (props.data != undefined) {
        // map data from home data and return productItems
        const mapped = props.data.map((n) => {
            return (<ProductItems clickHandler={() => addtoBasket(n.id)} key={n.id} class='productItems col-xs-12 col-s-6 col-m-4 col-l-3' src={n.image} alt={n.title} link={`/products/${n.id}`} text={n.title} price={n.price} />)
        })
        return (
            <>
                <section className='home_products container section-gap'>
                    <Title title='Lastest Products' text='Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.' />
                    <div>
                        {mapped}
                    </div>
                </section>
